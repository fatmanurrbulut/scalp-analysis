...
calibration_size = floor_pct = fallback_pct = None
...
analysis_ready = df_raw is not None and selected_pid is not None

analysis_tab, csv_tab = st.tabs(["📊 Hasta Analizi", "🗂️ CSV İncele"])

with csv_tab:
    from csv_inspector import render_csv_inspector_tab

    render_csv_inspector_tab()

with analysis_tab:
    if not analysis_ready:
        st.info("Hasta analizini görmek için soldan bir CSV yükleyip bir hasta seçin.")
        st.stop()
